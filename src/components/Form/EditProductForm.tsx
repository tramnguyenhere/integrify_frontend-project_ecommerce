...
import { deleteSingleProduct, fetchAllProducts, updateSingleProduct } from '../../redux/reducers/productsReducer';
...
const EditProductForm = () => {
    const { products, filteredProducts, loading, error } = useAppSelector(
      ...
      const toBeEditedProduct = products.filter(product=>product.id === Number(id))[0];
  ...
  const [newCategory, setNewCategory] = useState(toBeEditedProduct?.category!.id);
    ...
    <form className='form' onSubmit={editFormHandler}>
      ...
      <div className='form__group'>
        ...
        <select value={newCategory} onChange={e=>setNewCategory(Number(e.target.value))}>
          ...
          category.id !== 0 && <option key={category.id} value={category.id}>{category.name}</option>
          ))}
        </select>
      </div>
      <div className='form__group'>
      ...
      <button className='full-width-button__secondary' onClick={deleteProductHandler}>
       Delete
      </button>
    </form>
  )
